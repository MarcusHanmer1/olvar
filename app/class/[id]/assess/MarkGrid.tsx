...
export default function MarkGrid({
  ...
  function refreshAvg(qi: number) {
    ...
    // Only include valid (non-over-max) entered values in avg
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStatus(null);
        setSaveError(
          data.error || "Analysis failed. Marks have been saved — you can retry later."
        );
        return;
      }
    } catch {
      ...
      setStatus(null);
      setSaveError(
        "Network error during analysis. Marks have been saved — you can retry later."
      );
      return;
    }
  ...
  const CELL_W = 52; // px — fixed column width for mark cells
  const NAME_W = 200;
  const TOTAL_W = 96;
          ...
          <thead>
            ...
            <tr>
              ...
              {questions.map((q) => (
                ...
                >
                  <span
                    style={{ fontWeight: 700, fontSize: "12px", color: "#1c1c1a" }}
                  >
                    Q{q.number}
                  </span>
                  <br />
                  <span style={{ fontWeight: 400, fontSize: "10px", color: "#6b6b67" }}>
                    /{q.max_marks}
                  </span>
                </th>
              ))}
              ...
              >
                <span style={{ fontWeight: 700, fontSize: "12px", color: "#1c1c1a" }}>
                  Total
                </span>
                <br />
                <span style={{ fontWeight: 400, fontSize: "10px", color: "#6b6b67" }}>
